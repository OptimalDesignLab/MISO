...
{
   const char *options_file = "ns_walltest_options.json";
   ...
   try
   {
      ...
      auto solver = createSolver<NavierStokesSolver<2, entvar>>(opt_file_name,
                                                         move(smesh));
      solver->setInitialCondition(uinit_pert);
      solver->printSolution("ns_wall_init", 0);

      // get the initial density error
      double l2_error_init = (static_cast<NavierStokesSolver<2, entvar>&>(*solver)
      ...
      *out << "\ninitial residual norm = " << res_error << endl;
      ...
      solver->printSolution("ns_wall_final",0);
      // get the final density error
      double l2_error_final = (static_cast<NavierStokesSolver<2, entvar>&>(*solver)
                            .calcConservativeVarsL2Error(uexact, 0));
