#ifndef MFEM_LINEAR_EVOLVER_DG
#define MFEM_LINEAR_EVOLVER_DG
...
/// \note supports partially assembled mass bilinear form
    ...
    public:
        ...
        SystemOperator(BilinearForm *_mass, NonlinearForm *_res)
            : Operator(_mass->FESpace()->GetTrueVSize()),
              mass(_mass),
              res(_res), Jacobian(NULL), prec(_mass->FESpace()->GetFE(0)->GetDof(),
    ...
    EulerEvolver::EulerEvolver(BilinearForm *_mass, NonlinearForm *_res,
        ...
        : TimeDependentOperator(_mass->FESpace()->GetTrueVSize(),
                                start_time, type),
          mass(_mass),  prec(_mass->FESpace()->GetFE(0)->GetDof(),
             BlockILU::Reordering::MINIMUM_DISCARDED_FILL), res(_res), x_work(width), r_work1(mass->Height()), r_work2(height)
    {
        ...
        if (_mass != nullptr)
        {
            ...
            mass_solver.SetPreconditioner(prec);
            mass_solver.SetOperator(mass->SpMat());
