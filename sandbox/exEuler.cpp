...
#include "euler_cut.hpp"
#include "solver_cut.hpp"
using namespace std;
...
{
   const char *options_file = "exEuler_options.json";
   ...
   sol_ofs.close();
   meshsave.close();
   cout << "file name " << opt_file_name << endl;
   unique_ptr<CutSolver> solver(
         new CutEulerSolver<2, entvar>(opt_file_name, move(mesh)));
