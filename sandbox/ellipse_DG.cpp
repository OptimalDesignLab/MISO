...
{
    const char *options_file = "ellipse_DG_options.json";
   // 1. Initialize MPI.
    int num_procs, rank;
    MPI_Init(&argc, &argv);
    ...
    try
    {
        ...
        auto solver = createSolver<DGSolver<2, entvar>>(opt_file_name,
                                                         move(mesh));
        solver->setInitialCondition(uexact);

        /// get the initial density error
        double l2_error = (static_cast<DGSolver<2, entvar> &>(*solver)
