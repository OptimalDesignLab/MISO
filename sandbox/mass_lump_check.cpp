...
{
   const char *options_file = "mass_lump_check_options.json";
   int myid = 0;
   ...
   try
   {
      ...
      unique_ptr<AbstractSolver> solver(
         new EulerSolver<1, entvar>(opt_file_name, move(smesh)));
      //unique_ptr<AbstractSolver> solver(new EulerSolver<2>(opt_file_name, nullptr));

      solver->masslumpCheck(uexact);
...
void uexact(const Vector &x, Vector& q)
{
   ...
   // q(0) = 1.0; // 0th order
   // q(0) = x(1); // 1st order
   // q(0) = x(0)*x(0); // 2nd order
   // q(0) = x(0)*x(0)*x(0); // 3rd order
   // q(0) = x(0) * x(0) * x(0) * x(0);
   q(0) = x(0)/sqrt(x(0)*x(0) + x(1)*x(1)); // nonlinear
}
