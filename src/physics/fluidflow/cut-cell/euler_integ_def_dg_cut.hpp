#ifndef MACH_EULER_INTEG_DEF_DG_CUT
#define MACH_EULER_INTEG_DEF_DG_CUT
...
template <int dim>
void CutEulerDGIntegrator<dim>::calcFluxJacState(const mfem::Vector &dir,
                                                 const mfem::Vector &q,
...
template <int dim>
void CutEulerDGIntegrator<dim>::calcFluxJacDir(const mfem::Vector &dir,
                                               const mfem::Vector &q,
                                               mfem::DenseMatrix &flux_jac)
{
   // declare vectors of active input variables
...
double CutDGIsentropicVortexBC<dim, entvar>::calcBndryFun(
    const mfem::Vector &x,
...
template <int dim, bool entvar>
void CutDGIsentropicVortexBC<dim, entvar>::calcFlux(const mfem::Vector &x,
                                                    const mfem::Vector &dir,
                                                    const mfem::Vector &q,
...
template <int dim, bool entvar>
void CutDGIsentropicVortexBC<dim, entvar>::calcFluxJacState(
    const mfem::Vector &x,
    const mfem::Vector &dir,
    const mfem::Vector &q,
    mfem::DenseMatrix &flux_jac)
...
template <int dim, bool entvar>
void CutDGIsentropicVortexBC<dim, entvar>::calcFluxJacDir(
    const mfem::Vector &x,
    const mfem::Vector &dir,
    const mfem::Vector &q,
    mfem::DenseMatrix &flux_jac)
...
template <int dim, bool entvar>
double CutDGSlipWallBC<dim, entvar>::calcBndryFun(const mfem::Vector &x,
                                                  const mfem::Vector &dir,
                                                  const mfem::Vector &q)
{
   mfem::Vector flux_vec(q.Size());
...
template <int dim, bool entvar>
void CutDGSlipWallBC<dim, entvar>::calcFlux(const mfem::Vector &x,
                                            const mfem::Vector &dir,
                                            const mfem::Vector &q,
                                            mfem::Vector &flux_vec)
{
...
template <int dim, bool entvar>
void CutDGSlipWallBC<dim, entvar>::calcFluxJacState(const mfem::Vector &x,
                                                    const mfem::Vector &dir,
                                                    const mfem::Vector &q,
                                                    mfem::DenseMatrix &flux_jac)
{
...
template <int dim, bool entvar>
void CutDGSlipWallBC<dim, entvar>::calcFluxJacDir(const mfem::Vector &x,
                                                  const mfem::Vector &dir,
                                                  const mfem::Vector &q,
                                                  mfem::DenseMatrix &flux_jac)
{
...
template <int dim, bool entvar>
double CutDGFarFieldBC<dim, entvar>::calcBndryFun(const mfem::Vector &x,
                                                  const mfem::Vector &dir,
                                                  const mfem::Vector &q)
{
   mfem::Vector flux_vec(q.Size());
...
template <int dim, bool entvar>
void CutDGSlipFarFieldBC<dim, entvar>::calcFlux(const mfem::Vector &x,
                                                const mfem::Vector &dir,
                                                const mfem::Vector &q,
                                                mfem::Vector &flux_vec)
{
...
template <int dim, bool entvar>
void CutDGSlipFarFieldBC<dim, entvar>::calcFluxJacState(
    const mfem::Vector &x,
    const mfem::Vector &dir,
    const mfem::Vector &q,
    mfem::DenseMatrix &flux_jac)
...
template <int dim, bool entvar>
void CutDGSlipFarFieldBC<dim, entvar>::calcFluxJacDir(
    const mfem::Vector &x,
    const mfem::Vector &dir,
    const mfem::Vector &q,
    mfem::DenseMatrix &flux_jac)
...
template <int dim, bool entvar>
void CutDGFarFieldBC<dim, entvar>::calcFlux(const mfem::Vector &x,
                                            const mfem::Vector &dir,
                                            const mfem::Vector &q,
                                            mfem::Vector &flux_vec)
{
...
template <int dim, bool entvar>
void CutDGFarFieldBC<dim, entvar>::calcFluxJacState(const mfem::Vector &x,
                                                    const mfem::Vector &dir,
                                                    const mfem::Vector &q,
                                                    mfem::DenseMatrix &flux_jac)
{
...
template <int dim, bool entvar>
void CutDGFarFieldBC<dim, entvar>::calcFluxJacDir(const mfem::Vector &x,
                                                  const mfem::Vector &dir,
                                                  const mfem::Vector &q,
                                                  mfem::DenseMatrix &flux_jac)
{
...
template <int dim, bool entvar>
CutDGInterfaceIntegrator<dim, entvar>::CutDGInterfaceIntegrator(
...
template <int dim, bool entvar>
double CutDGInterfaceIntegrator<dim, entvar>::calcIFaceFun(
    const mfem::Vector &dir,
...
template <int dim, bool entvar>
void CutDGInterfaceIntegrator<dim, entvar>::calcFlux(const mfem::Vector &dir,
                                                     const mfem::Vector &qL,
                                                     const mfem::Vector &qR,
                                                     mfem::Vector &flux)
{
...
template <int dim, bool entvar>
void CutDGInterfaceIntegrator<dim, entvar>::calcFluxJacState(
    const mfem::Vector &dir,
    const mfem::Vector &qL,
    const mfem::Vector &qR,
...
template <int dim, bool entvar>
void CutDGInterfaceIntegrator<dim, entvar>::calcFluxJacDir(
    const mfem::Vector &dir,
    const mfem::Vector &qL,
    const mfem::Vector &qR,
...
template <int dim, bool entvar>
double CutDGPressureForce<dim, entvar>::calcBndryFun(const mfem::Vector &x,
                                                     const mfem::Vector &dir,
                                                     const mfem::Vector &q)
{
   calcSlipWallFlux<double, dim, entvar>(
...
template <int dim, bool entvar>
void CutDGPressureForce<dim, entvar>::calcFlux(const mfem::Vector &x,
                                               const mfem::Vector &dir,
                                               const mfem::Vector &q,
                                               mfem::Vector &flux_vec)
{
...
template <int dim, bool entvar>
void InviscidExactBC<dim, entvar>::calcFluxJacDir(const mfem::Vector &x,
                                                  const mfem::Vector &dir,
                                                  const mfem::Vector &q,
                                                  mfem::DenseMatrix &flux_jac)
{
...
template <int dim, bool entvar>
void CutDGInviscidExactBC<dim, entvar>::calcFluxJacState(
    const mfem::Vector &x,
    const mfem::Vector &dir,
    const mfem::Vector &q,
    mfem::DenseMatrix &flux_jac)
...
template <int dim, bool entvar>
void CutDGInviscidExactBC<dim, entvar>::calcFluxJacDir(
    const mfem::Vector &x,
    const mfem::Vector &dir,
    const mfem::Vector &q,
    mfem::DenseMatrix &flux_jac)
...
/// Potential flow functions start here
template <int dim, bool entvar>
double DGPotentialFlowBC<dim, entvar>::calcBndryFun(const mfem::Vector &x,
                                                    const mfem::Vector &dir,
                                                    const mfem::Vector &q)
{
   mfem::Vector flux_vec(q.Size());
...
template <int dim, bool entvar>
void DGPotentialFlowBC<dim, entvar>::calcFlux(const mfem::Vector &x,
                                              const mfem::Vector &dir,
                                              const mfem::Vector &q,
                                              mfem::Vector &flux_vec)
{
   calcPotentialFlowFlux<double, entvar>(
       x.GetData(), dir.GetData(), q.GetData(), flux_vec.GetData());
}

template <int dim, bool entvar>
void DGPotentialFlowBC<dim, entvar>::calcFluxJacState(
    const mfem::Vector &x,
    const mfem::Vector &dir,
    const mfem::Vector &q,
    mfem::DenseMatrix &flux_jac)
{
   ...
   // create container for active double flux output
   std::vector<adouble> flux_a(q.Size());
   mach::calcPotentialFlowFlux<adouble, entvar>(
       x_a.data(), dir_a.data(), q_a.data(), flux_a.data());
   this->stack.independent(q_a.data(), q.Size());
   this->stack.dependent(flux_a.data(), q.Size());
   this->stack.jacobian(flux_jac.GetData());
...
template <int dim, bool entvar>
void DGPotentialFlowBC<dim, entvar>::calcFluxJacDir(const mfem::Vector &x,
                                                    const mfem::Vector &dir,
                                                    const mfem::Vector &q,
                                                    mfem::DenseMatrix &flux_jac)
{
