 ...
 : AbstractSolver(json_options, move(smesh), comm)
{
   ...
   cout << "#mesh faces " << mesh->GetNumFaces() << endl;
...
/// ignore this for airfoil case
      ...
      if (tr->Elem2No < 0)
      {
         if (find(cutelems.begin(), cutelems.end(), tr->Elem1No) !=
             cutelems.end())
         {
            cout << "is there any boundary  face ? " << endl;
   ...
   {
      mass->AddDomainIntegrator(new CutDGMassIntegrator(
          cutSquareIntRules, embeddedElements, num_state));
   }
   ...
   void CutEulerDGSolver<dim, entvar>::addResVolumeIntegrators(double alpha)
   {
      ...
      cout << "correct area: " << (1600.0 - 0.0817073) << endl;
      cout << "calculated area: " << area << endl;
      cout << "area err " << endl;
      cout << abs(area - 1600.0 + 0.0817073) << endl;
...
/// use this for testing purposes
   ...
   res->AddDomainIntegrator(new CutDGIsentropicVortexBC<dim, entvar>(
       diff_stack, fec.get(), cutSegmentIntRules, phi, alpha));
   double peri_inner = res->GetEnergy(x);
   cout << "correct perimeter of airfoil: " << 2.03955 << endl;
   cout << "calculated perimeter: " << peri_inner - area << endl;
   cout << " +++++++++++++++++++++++++++++++++++++++++++++++++++++++ " << endl;
   cout << "area error: " << endl;
   cout << abs(area - 1600.0 + 0.0817073) << endl;
   cout << "airfoil perimeter error: " << endl;
   cout << abs(peri_inner - area - 2.03955) << endl;
   cout << " +++++++++++++++++++++++++++++++++++++++++++++++++++++++ " << endl;
#endif
   }
   ...
   template <int dim, bool entvar>
   void CutEulerDGSolver<dim, entvar>::iterationHook(
       int iter, double t, double dt, const ParGridFunction &state)
   {
      double entropy = ent->GetEnergy(state);
       ...
       const ParGridFunction &state) const
   {
      if (options["time-dis"]["steady"].template get<bool>())
      {
         ...
         if (norm <=
             options["time-dis"]["steady-abstol"].template get<double>())
         {
            return true;
         }
         if (norm <=
   ...
   template <int dim, bool entvar>
   void CutEulerDGSolver<dim, entvar>::terminalHook(
       int iter, double t_final, const ParGridFunction &state)
   {
      double entropy = ent->GetEnergy(state);
   ...
   void CutEulerDGSolver<dim, entvar>::getFreeStreamState(mfem::Vector & q_ref)
   {
      ...
      q_ref(dim + 1) =
          1 / (euler::gamma * euler::gami) + 0.5 * mach_fs * mach_fs;
   }
                            ...
                            const Vector &exact) -> double
      {
         if (entvar)
         {
            calcConservativeVars<double, dim>(discrete.GetData(),
                                              qdiscrete.GetData());
            calcConservativeVars<double, dim>(exact.GetData(),
                                              qexact.GetData());
         }
         else
         {
