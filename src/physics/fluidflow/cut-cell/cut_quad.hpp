...
struct circle
{
   ...
   T operator()(const blitz::TinyVector<T, N> &x) const
   {
      ...
      return -1 * ((((x[0] * xscale) + xmin) * ((x[0] * xscale) + xmin)) +
                   (((x[1] * yscale) + ymin) * ((x[1] * yscale) + ymin)) -
                   (radius * radius));
   }
   template <typename T>
...
template <int N>
bool cutByGeom(std::unique_ptr<mfem::Mesh> mesh, int &elemid)
{
   ...
   for (int i = 0; i < v.Size(); ++i)
   {
      ...
      lvsval(i) = ((coord[0] - xc) * (coord[0] - xc)) +
                  ((coord[1] - yc) * (coord[1] - yc)) - (r * r);
...
template <int N>
bool insideBoundary(std::unique_ptr<mfem::Mesh> mesh, int &elemid)
{
   Element *el = mesh->GetElement(elemid);
   Array<int> v;
   el->GetVertices(v);
...
template <int N>
void GetCutElementIntRule(Mesh *mesh,
                          vector<int> cutelems,
                          int order,
                          double radius,
                          std::map<int, IntegrationRule *> &cutSquareIntRules)
