...
/// Integrator for local projection stabilization for shock capturing
/// \tparam Derived - a class Derived from this one (needed for CRTP)
template <typename Derived>
class LPSShockIntegrator : public mfem::NonlinearFormIntegrator
{
public:
   /// Construct an LPS shock integrator
   /// \param[in] diff_stack - for algorithmic differentiation
   /// \param[in] num_state_vars - the number of state variables
   /// \param[in] a - used to move residual to lhs (1.0) or rhs(-1.0)
   /// \param[in] coeff - the LPS coefficient
