   ...
   res(fes, *fields)
{
   ...
   switch(dim) {
      case 1 : 
         addFluidIntegrators<1>(options["flow-param"], options["space_dis"]);
         break;
      case 2 :
         addFluidIntegrators<2>(options["flow-param"], options["space_dis"]);
         break;
      case 3 :
         addFluidIntegrators<3>(options["flow-param"], options["space_dis"]);
         break;
      default :
...
template <int dim>
void FluidResidual::addFluidIntegrators(
   ...
   const nlohmann::json &flow, const nlohmann::json &space_dis)
{
   ...
   auto lps_coeff = space_dis["lps-coeff"].get<double>();
   if (lps_coeff > 0.0)
   {
   ...
   const nlohmann::json &flow, const nlohmann::json &space_dis)
{
   ...
   {
      auto iface_coeff = space_dis["iface-coeff"].get<double>();
                ...
                const nlohmann::json &options)
{
   ...
   residual.iroll = options["flow-param"]["roll-axis"].get<int>();
   residual.ipitch = options["flow-param"]["pitch-axis"].get<int>();
   residual.state_is_entvar = false;
   if (options["flow-param"]["state"].get<string> == "entropy")
   {
      residual.state_is_entvar = true;
   }
