...
namespace mach
{
   ...
   void AbstractSolver::solveUnsteady(ParCentGridFunction &state)
   {
      ...
      //    state.Save(osol);
      // }
      double res_norm0 = calcResidualNorm(state);
       ...
       void (*pert_fun)(const mfem::Vector &, mfem::Vector &))
   {
      ...
      bool galerkin_diff = options["space-dis"].value("GD", false);
      switch (galerkin_diff)
      {
      case true:
      {
         cout << "checkin DGD jac " << endl;
         GDGridFunType u_plus(*uc);
         GDGridFunType u_minus(*uc);
...
// Warning: SetFromTrueDofs() not working for GD space. Need to revisit
#if 0
         res->Mult(*u_p, *res_p);
         res->Mult(*u_m, *res_m);
         ...
         jac_v -= res_plus;
         GridFunType jac_prod(fes.get());
         fes_GD->GetProlongationMatrix()->Mult(jac_v, jac_prod);
         double error = calcInnerProduct(jac_prod, jac_prod);
      ...
      default:
         throw mach::MachException(
             "Only two types of FESpace work at this point\n"
             "\tcannot handle given fespace");
      }
   }
