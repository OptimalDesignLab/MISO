...
public:
   ...
   friend void evaluate(MachNonlinearForm &form, const MachInputs &inputs, 
                        mfem::Vector &res_vec);
   ...
   /// Adds the given boundary integrator to the nonlinear form
   /// \param[in] integrator - nonlinear form integrator for boundary
   /// \tparam T - type of integrator, used for constructing MachIntegrator
   /// \note Assumes ownership of integrator
   template <typename T>
   void addBoundaryIntegrator(T *integrator);

   /// Adds given boundary integrator, restricted to the given boundary
   /// attributes.
   /// \param[in] integrator - nonlinear form integrator for boundary
...
template <typename T>
void MachNonlinearForm::addBoundaryIntegrator(T *integrator)
{
   integs.emplace_back(*integrator);
   nf.AddBoundaryIntegrator(integrator);
   //mach::addSensitivityIntegrator(*integrator, nf_fields, sens, scalar_sens);
}
                                           ...
                                           mfem::Array<int> &bdr_attr_marker)
{
   ...
   nf.AddBoundaryIntegrator(integrator, bdr_marker.back());
   //mach::addSensitivityIntegrator(*integrator, nf_fields, sens, scalar_sens);
}
