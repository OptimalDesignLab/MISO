...
{
public:
   /// \brief Define a demag flux (flux density B at the knee) model that is a linear function of temperature
   ...
   LinearTempDepDemagFluxCoefficient(const double &alpha_B_knee, const double &beta_B_knee);
   ...
   /// \brief Evaluate the second derivative of the demag flux in the element with respect to the 2nd state variable
   ///   described by trans at the point ip.
   /// \note When this method is called, the caller must make sure that the
   /// IntegrationPoint associated with trans is the same as ip. This can be
   /// achieved by calling trans.SetIntPoint(&ip).
    ...
    const nlohmann::json &materials)
{
   ...
   /// material. We default to a B_knee coeff of 0 ///TODO: (change this value as needed) unless
   ...
   else
   {
      ...
      if (material.contains("Demag"))
      {
        ...
        double beta_B_knee;
         ...
         getDemagParams(material, materials,
         ...
         //          "Insufficient information to compute demag flux coefficient for material \"";
...
double DemagFluxCoefficient::EvalStateDeriv(
    mfem::ElementTransformation &trans,
    const mfem::IntegrationPoint &ip,
    double state)
{
...
/// TODO: Change B_knee(std::make_unique<mfem::ConstantCoefficient>(0.0)) line IF the equivalent lines...
/// std::unique_ptr<mfem::Coefficient> constructDefaultDemagFluxCoeff( from earlier change
 ...
 : B_knee(std::make_unique<mfem::ConstantCoefficient>(0.0)) 
{
   ...
   {
      /// Options are being passed in. Loop over the components within and construct a demag flux coefficient for each
      for (const auto &component : B_knee_options["components"])
      {
         int attr = component.value("attr", -1);
         if (-1 != attr)
         {
            B_knee.addCoefficient(attr,
                              constructDefaultDemagFluxCoeff(component, materials));
         }
         else
         {
            for (const auto &attribute : component["attrs"])
            {
               B_knee.addCoefficient(attribute,
                                 constructDemagFluxCoeff(component, materials));
            }
         }
      }
   }
   ...
   {
      /// Components themselves are being passed in. Loop over the components and construct a demag flux coefficient for each
...
namespace
{
   LinearTempDepDemagFluxCoefficient::LinearTempDepDemagFluxCoefficient(const double &alpha_B_knee, const double &beta_B_knee)
   ...
   const double state)
{
   ...
   // std::cout << "B_knee = alpha_B_knee*T+beta_B_knee = " << alpha_B_knee << "*" << T << "+" << beta_B_knee << "\n";
