...
public:
   ...
   PMDemagConstraintEqCoeff(const double &T0,
                        ...
                        const double &alpha_B_knee, const double &beta_B_knee,
                        const double &alpha_H_knee, const double &beta_H_knee);

   /// \brief Evaluate the permanent magnetic demagnetization constraint equation coefficient in the element described by trans at the
   /// point ip.
   ...
   /// \brief Evaluate the derivative of the permanent magnetic demagnetization constraint equation coefficient in the element with respect to the 1st state variable
   ...
   /// \brief Evaluate the derivative of the permanent magnetic demagnetization constraint equation coefficient in the element with respect to the 2nd state variable
   ///   described by trans at the point ip.
   /// \note When this method is called, the caller must make sure that the
   /// IntegrationPoint associated with trans is the same as ip. This can be
   /// achieved by calling trans.SetIntPoint(&ip).
   ...
   /// \brief Evaluate the second derivative of the permanent magnetic demagnetization constraint equation coefficient in the element with respect to the 1st state variable
   ///   described by trans at the point ip.
   /// \note When this method is called, the caller must make sure that the
   /// IntegrationPoint associated with trans is the same as ip. This can be
   /// achieved by calling trans.SetIntPoint(&ip).
   ...
   /// \brief Evaluate the second derivative of the permanent magnetic demagnetization constraint equation coefficient in the element with respect to the 1st then 2nd state variable
   ///   described by trans at the point ip.
   /// \note When this method is called, the caller must make sure that the
   /// IntegrationPoint associated with trans is the same as ip. This can be
   /// achieved by calling trans.SetIntPoint(&ip).
   ...
   /// \brief Evaluate the derivative of the permanent magnetic demagnetization constraint equation coefficient in the element with respect to the 2nd then 1st state variable
   ///   described by trans at the point ip.
   /// \note When this method is called, the caller must make sure that the
   /// IntegrationPoint associated with trans is the same as ip. This can be
   /// achieved by calling trans.SetIntPoint(&ip).
   ...
   /// TODO: Adapt EvalRevDiff if needed for permanent magnetic demagnetization constraint equation coefficient
...
/// TODO: this default permanent magnetic demagnetization constraint coefficient will need to be altered
...
// Get Demag parameters (T0, alphas, betas, and values at reference temperatures) from JSON
void getDemagParams(const nlohmann::json &material,
                  const nlohmann::json &materials,
                  double &T0,
                  double &alpha_B_r, double &B_r_T0,
                  double &alpha_H_ci, double &H_ci_T0,
                  double &alpha_B_knee, double &beta_B_knee,
                  double &alpha_H_knee, double &beta_H_knee)
{
   ...
   // Assign alpha_H_ci based on material options, else refer to material library
   ...
   {
      alpha_H_ci = materials[material_name]["Demag"]["alpha_H_ci"].get<double>();
   }
   ...
   // Assign alpha_B_knee based on material options, else refer to material library
   ...
   {
      alpha_B_knee = materials[material_name]["Demag"]["alpha_B_knee"].get<double>();
   }

   // Assign beta_B_knee based on material options, else refer to material library
   ...
   {
      beta_B_knee = materials[material_name]["Demag"]["beta_B_knee"].get<double>();
   }

   // Assign alpha_H_knee based on material options, else refer to material library
   ...
   {
      alpha_H_knee = materials[material_name]["Demag"]["alpha_H_knee"].get<double>();
   }

   // Assign beta_H_knee based on material options, else refer to material library
   ...
   {
      beta_H_knee = materials[material_name]["Demag"]["beta_H_knee"].get<double>();
   }
}
    ...
    const nlohmann::json &materials)
{
   ...
   /// material. We default to a PMDemagConstraint coeff of 0 ///TODO: (change this value as needed) unless
   /// there is a different value in the material library
   if (material.is_string())
   {
      const auto &material_name = material.get<std::string>();
      temp_coeff = constructDefaultPMDemagConstraintCoeff(material_name, materials);
   }
   else
   {
      const auto &material_name = material["name"].get<std::string>();
      ...
      if (material.contains("Demag"))
      {
        ...
        double beta_H_knee;
         ...
         getDemagParams(material, materials, T0, 
                        alpha_B_r, B_r_T0, 
                        alpha_H_ci, H_ci_T0, 
                        alpha_B_knee, beta_B_knee, 
                        alpha_H_knee, beta_H_knee);
         ...
         //          "Insufficient information to compute permanent magnetic demagnetization constraint coefficient for material \"";
...
void PMDemagConstraintCoefficient::EvalRevDiff(const double Q_bar,
                                         mfem::ElementTransformation &trans,
                                         const mfem::IntegrationPoint &ip,
...
/// TODO: Change PMDemagConstraint(std::make_unique<mfem::ConstantCoefficient>(0.0)) line IF the equivalent lines...
/// std::unique_ptr<mfem::Coefficient> constructDefaultPMDemagConstraintCoeff( from earlier change
PMDemagConstraintCoefficient::PMDemagConstraintCoefficient(const nlohmann::json &PMDemagConstraint_options,
                                               const nlohmann::json &materials)
 : PMDemagConstraint(std::make_unique<mfem::ConstantCoefficient>(0.0)) 
{
   if (PMDemagConstraint_options.contains("components"))
   {
      /// Options are being passed in. Loop over the components within and construct a PMDemagConstraint coefficient for each
      for (const auto &component : PMDemagConstraint_options["components"])
      {
         int attr = component.value("attr", -1);
         if (-1 != attr)
         {
            PMDemagConstraint.addCoefficient(attr,
         ...
         else
         {
            ...
            {
               PMDemagConstraint.addCoefficient(attribute,
   ...
   {
      /// Components themselves are being passed in. Loop over the components and construct a PMDemagConstraint coefficient for each
...
namespace
{
   PMDemagConstraintEqCoeff::PMDemagConstraintEqCoeff(const double &T0,
                        ...
                        const double &alpha_B_knee, const double &beta_B_knee,
                        const double &alpha_H_knee, const double &beta_H_knee)
   : T0(T0), alpha_B_r(alpha_B_r), B_r_T0(B_r_T0), alpha_H_ci(alpha_H_ci), H_ci_T0(H_ci_T0), alpha_B_knee(alpha_B_knee), beta_B_knee(beta_B_knee), alpha_H_knee(alpha_H_knee), beta_H_knee(beta_H_knee)
{
...
double PMDemagConstraintEqCoeff::Eval2ndDerivS2S1(mfem::ElementTransformation &trans,
                           const mfem::IntegrationPoint &ip,
                           const double state1,
                           const double state2)
{
