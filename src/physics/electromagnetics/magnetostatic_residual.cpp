...
void setInputs(MagnetostaticResidual &residual, const miso::MISOInputs &inputs)
{
...
void linearize(MagnetostaticResidual &residual, const miso::MISOInputs &inputs)
{
   ...
   prec(constructPreconditioner(fes, options["lin-prec"]))
{
   ...
   else if (space_dim == 2)
   {
      ...
      MISOLinearForm linear_form(fes, fields);
      if (options.contains("current"))
      {
         current_coeff = std::make_unique<CurrentDensityCoefficient2D>(
             diff_stack, options["current"]);

         // MISOInputs current_inputs = {{"current_density:phaseA", 0.0},
         ...
         linear_form.addDomainIntegrator(
             new miso::DomainLFIntegrator(*current_coeff), current_attrs);
      }
      if (options.contains("magnets"))
      {
         ...
         nuM = std::make_unique<miso::ScalarVectorProductCoefficient>(
             nu, *mag_coeff);
      ...
      load = std::make_unique<MISOLoad>(std::move(linear_form));
   }
   else
   {
      throw MISOException(
