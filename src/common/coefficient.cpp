...
void HashinShtrikmanWeightedCoefficient::setInputs(const MISOInputs &inputs)
{
...
void dHashinShtrikmanWeightedCoefficient::setInputs(const MISOInputs &inputs)
{
   setValueFromInputs(inputs, weighted_by, weight);
}
...
void WeightedAverageCoefficient::setInputs(const MISOInputs &inputs)
{
   setValueFromInputs(inputs, weighted_by, weight);
}
...
void dWeightedAverageCoefficient::setInputs(const MISOInputs &inputs)
{
   setValueFromInputs(inputs, weighted_by, weight);
}
...
void MeshDependentCoefficient::setInputs(const MISOInputs &inputs)
{
...
std::unique_ptr<miso::MeshDependentCoefficient> constructMaterialCoefficient(
    ...
    double default_val)
{
   auto material_coeff = std::make_unique<miso::MeshDependentCoefficient>();
   ...
   for (const auto &component : components)
   {
      ...
      if (materials[material_name].contains(name))
      {
         ...
         else if (materials[material_name][name].is_object())
         {
            ...
            {
               throw MISOException("unexpected type for comp_materials!\n");
            }
            ...
            if (weight == "Hashin-Shtrikman")
            {
               if (-1 != attr)
               {
                  auto coeff = std::make_unique<
                      miso::HashinShtrikmanWeightedCoefficient>(
            ...
            else if (weight == "dHashin-Shtrikman")
            {
               ...
               {
                  auto coeff = std::make_unique<
                      miso::dHashinShtrikmanWeightedCoefficient>(
                      vals[0], vals[1], weighted_by);
                  material_coeff->addCoefficient(attr, move(coeff));
               }
               else
            ...
            else if (weight == "average")
            {
               ...
               {
                  auto coeff =
                      std::make_unique<miso::WeightedAverageCoefficient>(
                          vals[0], vals[1], weighted_by);
                  material_coeff->addCoefficient(attr, move(coeff));
               }
               else
            ...
            else if (weight == "daverage")
            {
               ...
               {
                  auto coeff =
                      std::make_unique<miso::dWeightedAverageCoefficient>(
                          vals[0], vals[1], weighted_by);
                  material_coeff->addCoefficient(attr, move(coeff));
               }
               else
...
{
   auto material_coeff =
       std::make_unique<miso::MeshDependentTwoStateCoefficient>();
   /// loop over all components, construct coeff for each
   for (const auto &component : components)
   {
      int attr = component.value("attr", -1);
...
{
   auto material_coeff =
       std::make_unique<miso::MeshDependentThreeStateCoefficient>();
   /// loop over all components, construct coeff for each
   for (const auto &component : components)
   {
      int attr = component.value("attr", -1);
