                         ...
                         xdouble *mat_vec)
{
   ...
   else
   {
      ...
      mat_vec[i + 1] -= (2 / 3) * T * mu * (q[j + 1] / q[0]) * vec[dim + 1];
      mat_vec[j + 1] = T * mu * vec[i + 1];
      mat_vec[j + 1] += T * mu * (q[i + 1] / q[0]) * vec[dim + 1];
      // get last entry of `mat_vec`
      mat_vec[dim + 1] = T * mu * (q[j + 1] / q[0]) * vec[i + 1];
      mat_vec[dim + 1] -= (2 / 3) * T * mu * (q[i + 1] / q[0]) * vec[j + 1];
      mat_vec[dim + 1] += (1 / 3) * T * mu * (q[i + 1] / q[0]) * (q[j + 1] / q[0]) * vec[dim + 1];
   }
}
