...
/// (if defined)
template <typename T>
struct function<T, std::void_t<decltype(&T::operator())>> : public function<decltype(&T::operator())>
{ };
...
/// converted to a function
template <typename T>
struct is_callable<T, std::void_t<typename detail::function<T>::type>> : std::true_type
{ };
