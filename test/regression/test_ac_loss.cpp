...
TEST_CASE("ACLossFunctionalIntegrator - Reddy Paper")
{
   ...
   std::unique_ptr<mfem::Mesh> mesh( new mfem::Mesh(
      ...
      mfem::Mesh::MakeCartesian3D(64, 128, 1,
   ...
   mfem::Vector peak_flux(solver.getFieldSize("flux_magnitude"));
   solver.calcOutput("flux_magnitude", {{"state", state}}, peak_flux);

   nlohmann::json ac_loss_options{{"num_strands", 10.0 * 18.0}};
   solver.createOutput("ac_loss", ac_loss_options);
