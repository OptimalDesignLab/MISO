...
//          auto &stack = miso::getDiffStack();
         
//          miso::MachLinearForm load(fes, fields);

//          miso::CurrentDensityCoefficient2D current_coeff(stack, options["current"]);
...
//          load.addDomainIntegrator(new miso::DomainLFIntegrator(current_coeff));

//          // miso::MagnetostaticResidual res(stack, fes, fields, options, materials, nu);
...
TEST_CASE("MagnetostaticResidual sensitivity wrt current_density")
{
   ...
   for (int p = 1; p <= 4; ++p)
   {
      DYNAMIC_SECTION( "...for degree p = " << p )
      {
         ...
         fields.emplace("dirichlet_bc", miso::FiniteElementState(mesh, fes, "dirichlet_bc"));
         ...
         fields.emplace("temperature",
                        miso::FiniteElementState(mesh, miso::FiniteElementState::Options{.order=p, .name="temperature"}));
