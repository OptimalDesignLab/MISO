...
TEST_CASE("EulerIntegrator::AssembleElementGrad3D", "[EulerIntegrator]")
{
   ...
   {  
      ...
      {  
         ...
         NonlinearForm res(fes.get());
         res.AddDomainIntegrator(new mach::EulerIntegrator<dim>(diff_stack));
                       ...
                       ((bool entvar), entvar), false, true)
{
   ...
   for (int p = 0; p <= 1; ++p)
   {
      DYNAMIC_SECTION("...for degree p = " << p)
      {
         ...
         NonlinearForm res(fes.get());
         res.AddDomainIntegrator(
            new mach::IsmailRoeIntegrator<dim,entvar>(diff_stack));
                       ...
                       ((bool entvar), entvar), false, true)
{
   ...
   for (int p = 0; p <= 1; ++p)
   {
      DYNAMIC_SECTION("...for degree p = " << p)
      {
         ...
         NonlinearForm res(fes.get());
         res.AddDomainIntegrator(
             new mach::EntStableLPSIntegrator<dim, entvar>(diff_stack));
                       ...
                       "[MassIntegrator]", ((bool entvar), entvar), false, true)
{
   ...
   for (int p = 0; p <= 1; ++p)
   {
      DYNAMIC_SECTION("...for degree p = " << p)
      {
         ...
         NonlinearForm res(fes.get());
         res.AddDomainIntegrator(
             new mach::MassIntegrator<dim, entvar>(diff_stack));
