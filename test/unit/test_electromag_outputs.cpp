...
TEST_CASE("FluxLinkageOutput sensitivity wrt state")
{
   using namespace mfem;
   using namespace electromag_data;
   ...
   for (int p = 1; p <= 4; ++p)
   {
      DYNAMIC_SECTION( "...for degree p = " << p )
      {
         ...
         mach::FluxLinkageOutput fun(mach::getDiffStack(), fes, fields, options, current_opts);
         mach::MachInputs inputs{
            {"mesh_coords", mesh_coords_tv},
            {"temperature", temp_tv},
            {"state", state_tv}
         ...
         state_tv.Add(delta, pert_vec); // remember to reset the mesh nodes
         std::cout << "dfdp_fwd: " << dfdp_fwd << " dfdp_fd: " << dfdp_fd << " dfdp_rev: " << dfdp_rev << "\n";
...
TEST_CASE("FluxLinkageOutput sensitivity wrt mesh_coords")
{
   using namespace mfem;
   using namespace electromag_data;
