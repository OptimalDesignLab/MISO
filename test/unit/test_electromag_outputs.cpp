...
TEST_CASE("DCLossFunctional::jacobianVectorProduct wrt mesh_coords")
{
...
//          mfem::Vector wrt_bar_fd(mesh_coords.space().GetTrueVSize());
//          wrt_bar_fd = 0.0;
...
//             // double dfdx_v_fd = adjoint_tv(0) * (dfdx_v_fd_p - dfdx_v_fd_m) / (2 * delta);
//             wrt_bar_fd(i) = adjoint_tv(0) * (dfdx_v_fd_p - dfdx_v_fd_m) / (2 * delta);

//             mesh_coords_tv.Add(delta, v_tv); // remember to reset the mesh nodes
//          }

//          std::cout << "wrt_bar:\t";
//          wrt_bar.Print(mfem::out, wrt_bar.Size());

//          std::cout << "wrt_bar_fd:\t";
//          wrt_bar_fd.Print(mfem::out, wrt_bar.Size());

//          for (int i = 0; i < v_tv.Size(); ++i)
//          {
//             REQUIRE(wrt_bar(i) == Approx(wrt_bar_fd(i)).margin(1e-8));
//          }
