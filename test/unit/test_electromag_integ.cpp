...
class TestMagnetizationSource : public mach::VectorStateCoefficient
{
public:
   ...
   : mach::VectorStateCoefficient(vdim)
   {}
};
...
TEST_CASE("NonlinearDGDiffusionIntegratorMeshRevSens::AssembleRHSElementVect")
{
   ...
   for (int p = 1; p <= 4; ++p)
   {
      DYNAMIC_SECTION( "...for degree p = " << p )
      {
         ...
         NonlinearForm res(&fes);
         auto *integ = new mach::NonlinearDGDiffusionIntegrator(one_sc, bc_val, mu);
...
TEST_CASE("TestBoundaryIntegratorMeshRevSens::AssembleRHSElementVect")
{
   ...
   for (int p = 1; p <= 4; ++p)
   {
      DYNAMIC_SECTION( "...for degree p = " << p )
      {
         ...
         NonlinearForm res(&fes);
         auto *integ = new mach::TestBoundaryIntegrator;
         res.AddBdrFaceIntegrator(integ);
...
TEST_CASE("DCLossDistributionIntegratorMeshRevSens::AssembleRHSElementVect")
{
   ...
   for (int p = 1; p <= 4; ++p)
   {
      DYNAMIC_SECTION("...for degree p = " << p)
      {
         ...
         LinearForm dfdx(&mesh_fes);
         dfdx.AddDomainIntegrator(
            new mach::DCLossDistributionIntegratorMeshRevSens(adjoint, *integ));
         dfdx.Assemble();
         double dfdx_v = dfdx * v;
...
TEST_CASE("DCLossDistributionIntegratorWindingVolumeRevSens::GetElementEnergy")
{
   ...
   for (int p = 1; p <= 4; ++p)
   {
      DYNAMIC_SECTION("...for degree p = " << p)
      {
         ...
         NonlinearForm dfdx(&temp_fes);
         dfdx.AddDomainIntegrator(
            new mach::DCLossDistributionIntegratorWindingVolumeRevSens(adjoint, *integ));
...
TEST_CASE("ACLosslDistributionIntegratorMeshRevSens::AssembleRHSElementVect")
{
   ...
   for (int p = 1; p <= 4; ++p)
   {
      DYNAMIC_SECTION("...for degree p = " << p)
      {
         ...
         LinearForm dfdx(&mesh_fes);
         dfdx.AddDomainIntegrator(
            new mach::ACLossDistributionIntegratorMeshRevSens(adjoint, *integ));
         dfdx.Assemble();
         double dfdx_v = dfdx * v;
...
TEST_CASE("ACLosslDistributionIntegratorPeakFluxRevSens::AssembleRHSElementVect")
{
   ...
   for (int p = 1; p <= 4; ++p)
   {
      DYNAMIC_SECTION("...for degree p = " << p)
      {
         ...
         dfdx.AddDomainIntegrator(
            new mach::ACLossDistributionIntegratorPeakFluxRevSens(adjoint, *integ));
         dfdx.Assemble();
         double dfdx_v = dfdx * v;
...
TEST_CASE("ACLosslDistributionIntegratorTemperatureRevSens::AssembleRHSElementVect")
{
   ...
   for (int p = 1; p <= 4; ++p)
   {
      DYNAMIC_SECTION("...for degree p = " << p)
      {
         ...
         LinearForm dfdx(&fes);
         dfdx.AddDomainIntegrator(
            new mach::ACLossDistributionIntegratorTemperatureRevSens(adjoint, *integ));
         dfdx.Assemble();
         double dfdx_v = dfdx * v;
...
TEST_CASE("ACLossDistributionIntegratorWindingVolumeRevSens::GetElementEnergy")
{
   ...
   for (int p = 1; p <= 4; ++p)
   {
      DYNAMIC_SECTION("...for degree p = " << p)
      {
         ...
         NonlinearForm dfdx(&fes);
         dfdx.AddDomainIntegrator(
            new mach::ACLossDistributionIntegratorWindingVolumeRevSens(adjoint, *integ));
...
TEST_CASE("CAL2CoreLossDistributionIntegratorMeshRevSens::AssembleRHSElementVect")
{
   ...
   for (int p = 1; p <= 4; ++p)
   {
      DYNAMIC_SECTION("...for degree p = " << p)
      {
         ...
         LinearForm dfdx(&mesh_fes);
         dfdx.AddDomainIntegrator(
            new mach::CAL2CoreLossDistributionIntegratorMeshRevSens(adjoint, *integ));
         dfdx.Assemble();
         double dfdx_v = dfdx * v;
...
TEST_CASE("CAL2CoreLossDistributionIntegratorPeakFluxRevSens::AssembleRHSElementVect")
{
   ...
   for (int p = 1; p <= 4; ++p)
   {
      DYNAMIC_SECTION("...for degree p = " << p)
      {
         ...
         LinearForm dfdx(&flux_fes);
         dfdx.AddDomainIntegrator(
            new mach::CAL2CoreLossDistributionIntegratorPeakFluxRevSens(adjoint, *integ));
         dfdx.Assemble();
         double dfdx_v = dfdx * v;
...
TEST_CASE("CAL2CoreLossDistributionIntegratorTemperatureRevSens::AssembleRHSElementVect")
{
   ...
   for (int p = 1; p <= 4; ++p)
   {
      DYNAMIC_SECTION("...for degree p = " << p)
      {
         ...
         LinearForm dfdx(&fes);
         dfdx.AddDomainIntegrator(
            new mach::CAL2CoreLossDistributionIntegratorTemperatureRevSens(adjoint, *integ));
         dfdx.Assemble();
         double dfdx_v = dfdx * v;
...
//          std::unique_ptr<mach::TwoStateCoefficient> TwoStateCoefficient_Constraint(new SimpleTwoStateCoefficient());

//          // Define the functional integrator that will be used to compute C(B,T)
//          auto *integ = new mach::PMDemagIntegrator(*TwoStateCoefficient_Constraint,&temperature_field_test);
