...
TEST_CASE("VolumeFunctional::jacobianVectorProduct wrt mesh_coords")
{
   ...
   for (int p = 1; p <= 4; ++p)
   {
      DYNAMIC_SECTION( "...for degree p = " << p )
      {
         ...
         miso::VolumeFunctional fun(fields, {});
...
TEST_CASE("MassFunctional::jacobianVectorProduct wrt mesh_coords")
{
   ...
   for (int p = 1; p <= 4; ++p)
   {
      DYNAMIC_SECTION( "...for degree p = " << p )
      {
         ...
         miso::MassFunctional fun(fields, components, materials, {});
         miso::MISOInputs inputs{
            {"mesh_coords", mesh_coords_tv}
         };
...
TEST_CASE("MassFunctional::vectorJacobianProduct wrt fill_factor")
{
   using namespace mfem;
   using namespace electromag_data;
   ...
   for (int p = 1; p <= 1; ++p)
   {
      DYNAMIC_SECTION( "...for degree p = " << p )
      {
         ...
         adjoint_tv(0) = 1.0;
         setInputs(fun, inputs);
         double wrt_bar = vectorJacobianProduct(fun, adjoint_tv, "fill_factor");
         double dfdx_v = wrt_bar;

         // now compute the finite-difference approximation...
         inputs["fill_factor"] = fill_factor + delta;
         double dfdx_v_fd_p = calcOutput(fun, inputs);
         // std::cout << "dfdx_v_fd_p: " << dfdx_v_fd_p << "\n";
...
TEST_CASE("StateAverageFunctional sensitivity wrt state")
{
   using namespace mfem;
   using namespace electromag_data;
   ...
   for (int p = 1; p <= 4; ++p)
   {
      DYNAMIC_SECTION( "...for degree p = " << p )
      {
         ...
         miso::StateAverageFunctional fun(state.space(), fields);
         miso::MISOInputs inputs{
            {"state", state_tv},
            {"mesh_coords", mesh_coords_tv}
         };
...
TEST_CASE("StateAverageFunctional sensitivity wrt mesh_coords")
{
   using namespace mfem;
   using namespace electromag_data;
...
TEST_CASE("IEAggregateFunctional sensitivity wrt state")
{
   ...
   for (int p = 1; p <= 4; ++p)
   {
      DYNAMIC_SECTION( "...for degree p = " << p )
      {
         ...
         miso::IEAggregateFunctional fun(state.space(), fields, fun_opts);
         miso::MISOInputs inputs{
            {"state", state_tv},
            {"mesh_coords", mesh_coords_tv}
         };
