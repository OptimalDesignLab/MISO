/* This script tests the newton method with a linesearch solving
   a nonlinear problem:
            F(x) = -1/3 * x^3 + x^2 - x = -1
*/
...
nonlinearFunc::nonlinearFunc(int s) : Operator(s), Jac(s) { }
...
TEST_CASE(" Use Newton Method with line search solving 1D problem...",
          "[linesearch-newton]")
{
   ...
   // declare the linesearch newton solver.
   mfem::LineSearchNewton linesearch_test;
   linesearch_test.iterative_mode = true;
   linesearch_test.SetPrintLevel(1);
   linesearch_test.SetAbsTol(abs_tol);
   linesearch_test.SetRelTol(rel_tol);
   linesearch_test.SetMaxIter(100);
   ...
   {
      linesearch_test.SetOperator(*quadfunc);
      quadfunc->Mult(x, r);
   ...
   SECTION("Set Solver, and check the initial derivative.  ")
   {
      ...
      linesearch_test.SetSolver(*J_solve);
      linesearch_test.GetSolver()->Mult(r, c);
      REQUIRE(temp1(0) == der);
      REQUIRE(c(0) == r(0) / der);
   }
   ...
   SECTION(" Solve simply equation ")
   {
      ...
      linesearch_test.Mult(b, x);
      REQUIRE(linesearch_test.GetConverged() == 1);
   }
   delete quadfunc;
   delete J_solve;
}
