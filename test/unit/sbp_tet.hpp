...
/// Class for (diagonal-norm) summation-by-parts operator on squares
class SBPSquareElement : public SBPFiniteElement
{
public:
   /// Constructor for SBP operator on squares (so-called "diagonal E")
   /// \param[in] degree - maximum poly degree for which operator is exact
   /// \param[in] num_nodes - the number of nodes the operator has
   SBPSquareElement(const int degree, const int num_nodes);
...
/// Class for (diagonal-norm) summation-by-parts operator on Tetrahedrons
class SBPTetrahedronElement : public SBPFiniteElement
{
public:
   /// Constructor for SBP operator on Tetrahedrons (so-called "diagonal E")
   /// \param[in] degree - maximum poly degree for which operator is exact
   /// \param[in] num_nodes - the number of nodes the operator has
   SBPTetrahedronElement(const int degree, const int num_nodes);
