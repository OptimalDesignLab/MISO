...
TEST_CASE("EulerDiffusionIntegrator::AssembleElementGrad",
          "[EulerDiffusionIntegrator]")
{
   using namespace mfem;
   using namespace euler_data;
   ...
   for (int p = 1; p <= 4; ++p)
   {
      DYNAMIC_SECTION("...for degree p = " << p)
      {
         ...
         res.AddBdrFaceIntegrator(
             new mach::DGSlipWallBC<dim>(diff_stack, fec.get()));
                       ...
                       true)
{
   ...
   {
      DYNAMIC_SECTION("Jacobian of Interface flux w.r.t state is correct (DSBP)"
                      << p)
      {
         std::unique_ptr<FiniteElementCollection> fec(
             new DG_FECollection(p, dim));
         ...
         res.AddInteriorFaceIntegrator(
             new mach::DGInterfaceIntegrator<dim, entvar>(
                 diff_stack, diss_coeff, fec.get()));

         // initialize state; here we randomly perturb a constant state
         GridFunction w(fes.get());
         VectorFunctionCoefficient pert(num_state,
                                        randBaselineVectorPert<dim, entvar>);
