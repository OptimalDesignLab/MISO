...
public:
   ...
   // v = 1 and u = (x + y)^2 and integrates over [0,1]^2.  This should give a
...
void setInputs(TestIntegrator &integ, const mach::MachInputs &inputs)
{
   ...
   if (it != inputs.end())
   {
      ...
      {
         throw mach::MachException("Bad input type for time!");
      }
   }
}
          ...
          "[MachResidual]")
{
   ...
   // These steps create a grid function for (x+y)^2
   std::function<double(const mfem::Vector&)> xy_fun = 
      [](const mfem::Vector&xy) { return xy(0) + xy(1); };
   auto coeff = FunctionCoefficient(xy_fun);
