                        ...
                        ((int dim), dim), 1, 2, 3 )
{
   ...
   SECTION( "Jacobian of Spectral radius of flux Jacobian is correct" )
   {
	  ...
	  // FD approximation
	  Jac_v_fd = (mach::calcSpectralRadiusJacDir(dir + delta*v, q) - 
				  mach::calcSpectralRadiusJacDir(dir - delta*v, q))/
				  (2*delta);
	  
	  REQUIRE( Jac_v_ad == Approx(Jac_v_fd) );
   }
