                       ...
                       ((int dim), dim), 1, 2, 3)
{
    ...
    for (int di = 0; di < dim; ++di)
    {
        DYNAMIC_SECTION("Apply scaling jacobian w.r.t state is correct")
        {
            ...
            esviscousinteg.applyScalingJacState(di, x, q, v_mat, mat_vec_jac);
            //mat_vec_jac.Print();
                       ...
                       ((int dim), dim), 2)
{
    ...
    for (int p = 1; p <= max_degree; ++p)
    {
        ...
        {
            mfem::DenseMatrix mat_vec_jac(num_states);
            noslipadiabatic.calcFluxJacState(x, nrm, jac, q, v_mat, mat_vec_jac);
            //mat_vec_jac.Print();
            // loop over each state variable and check column of mat_vec_jac...
            for (int i = 0; i < num_states; ++i)
            {
                ...
                q_plus(i) += delta;
                q_minus(i) -= delta;
                noslipadiabatic.calcFlux(x, nrm, jac, q_plus, v_mat, mat_vec_plus);
                noslipadiabatic.calcFlux(x, nrm, jac, q_minus, v_mat, mat_vec_minus);
                mfem::Vector mat_vec_fd(num_states);
                mat_vec_fd = 0.0;
                subtract(mat_vec_plus, mat_vec_minus, mat_vec_fd);
                mat_vec_fd /= 2.0 * delta;
...
TEMPLATE_TEST_CASE_SIG("Slip wall Jacobian", "[ViscousSlipWallBC]",
                       ((int dim), dim), 2)
{
    using namespace euler_data;
    double delta = 1e-5;
    ...
    for (int p = 1; p <= max_degree; ++p)
    {
        ...
        DYNAMIC_SECTION("jacobian of Viscous Slip Wall BC w.r.t state is correct")
        {
            ...
            for (int i = 0; i < num_states; ++i)
            {
                ...
                q_plus(i) += delta;
                q_minus(i) -= delta;
                viscousslipwall.calcFlux(x, nrm, jac, q_plus, v_mat, mat_vec_plus);
                viscousslipwall.calcFlux(x, nrm, jac, q_minus, v_mat, mat_vec_minus);
                mfem::Vector mat_vec_fd(num_states);
                mat_vec_fd = 0.0;
                subtract(mat_vec_plus, mat_vec_minus, mat_vec_fd);
                mat_vec_fd /= 2.0 * delta;
    ...
    for (int p = 1; p <= max_degree; ++p)
    {
        ...
        DYNAMIC_SECTION("jacobian of viscous inflow bc w.r.t state is correct")
        {
            ...
            for (int i = 0; i < num_states; ++i)
            {
                ...
                q_plus(i) += delta;
                q_minus(i) -= delta;
                viscousinflow.calcFlux(x, nrm, jac, q_plus, v_mat, mat_vec_plus);
                viscousinflow.calcFlux(x, nrm, jac, q_minus, v_mat, mat_vec_minus);
                mfem::Vector mat_vec_fd(num_states);
                mat_vec_fd = 0.0;
                subtract(mat_vec_plus, mat_vec_minus, mat_vec_fd);
                mat_vec_fd /= 2.0 * delta;
    ...
    for (int p = 1; p <= max_degree; ++p)
    {
        ...
        DYNAMIC_SECTION("jacobian of viscous outflow bc w.r.t state is correct")
        {
            ...
            for (int i = 0; i < num_states; ++i)
            {
                ...
                q_plus(i) += delta;
                q_minus(i) -= delta;
                viscousoutflow.calcFlux(x, nrm, jac, q_plus, v_mat, mat_vec_plus);
                viscousoutflow.calcFlux(x, nrm, jac, q_minus, v_mat, mat_vec_minus);
                mfem::Vector mat_vec_fd(num_states);
                mat_vec_fd = 0.0;
                subtract(mat_vec_plus, mat_vec_minus, mat_vec_fd);
                mat_vec_fd /= 2.0 * delta;
    ...
    for (int p = 1; p <= max_degree; ++p)
    {
        ...
        DYNAMIC_SECTION("jacobian of viscous farfield bc w.r.t state is correct")
        {
            ...
            for (int i = 0; i < num_states; ++i)
            {
                ...
                q_plus(i) += delta;
                q_minus(i) -= delta;
                viscousfarfield.calcFlux(x, nrm, jac, q_plus, v_mat, mat_vec_plus);
                viscousfarfield.calcFlux(x, nrm, jac, q_minus, v_mat, mat_vec_minus);
                mfem::Vector mat_vec_fd(num_states);
                mat_vec_fd = 0.0;
                subtract(mat_vec_plus, mat_vec_minus, mat_vec_fd);
                mat_vec_fd /= 2.0 * delta;
