import copy
from miso.omMeshMovement import omMeshMove

from mphys import Builder
import numpy as np
import openmdao.api as om

from .pyMISO import MISOSolver

class omMISOState(om.ImplicitComponent):
...
class omMISOFunctional(om.ExplicitComponent):
...
class MISOCouplingGroup(om.Group):
    ...
    def setup(self):
        ...
        self.add_subsystem("solver",
                           MISOState(solver=self.solver, check_partials=self.check_partials),
                           promotes_inputs=[("mesh_coords", "miso_vol_coords")],
                           promotes_outputs=[("state", "miso_states")])
...
class MISOMeshGroup(om.Group):
    ...
    def setup(self):
        solver = self.options['solver']
        self.add_subsystem('mesh', MISOMesh(solver=solver))
...
class MISOBuilder(Builder):
    ...
    def initialize(self, comm):
        ...
        # Create miso solver instance
        self.comm = comm
        self.solver = MISOSolver(options=solver_options, comm=comm)
    ...
    def get_pre_coupling_subsystem(self, scenario_name=None):
        ...
        return MISOPrecouplingGroup(solver=self.solver, initial_dv_vals=initial_dvs)

    def get_post_coupling_subsystem(self, scenario_name=None):
        return MISOOutputsGroup(
            solver=self.solver,
            check_partials=self.check_partials,
            scenario_name=scenario_name,
            problem_setup=self.problem_setup
