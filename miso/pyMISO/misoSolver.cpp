...
void initSolver(py::module &m)
{
   ...
   py::class_<AbstractSolver2>(m, "MISOSolver")
       //  .def(py::init(
       ...
       .def(
           ...
           {
              auto state_vec = npBufferToMFEMVector(state);
              self.solveForState(pyDictToMISOInputs(py_inputs), state_vec);
           },
           py::arg("inputs"),
           py::arg("state"))
       .def(
       ...
       .def(
           ...
           {
              auto res_vec = npBufferToMFEMVector(residual);
              self.calcResidual(pyDictToMISOInputs(py_inputs), res_vec);
           },
           py::arg("inputs"),
           py::arg("residual"))
       ...
       .def(
           ...
           { return self.calcResidualNorm(pyDictToMISOInputs(py_inputs)); },
           py::arg("inputs"))
       ...
       .def(
           ...
           [](AbstractSolver2 &self,
              ...
              const py::dict &py_inputs)
           { return self.calcOutput(output, pyDictToMISOInputs(py_inputs)); },
       ...
       .def(
              ...
              const py::array_t<double> &out)
           {
              ...
              self.calcOutput(output, pyDictToMISOInputs(py_inputs), out_vec);
           },
